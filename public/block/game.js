class Game{
	constructor(){
		...
		const game = this;
		this.anims = [ 'Walking Backwards', 'Turn', 'walkingf', 'Pointing', 'walkingf',];
		...
		const options = {
			assets:[
				...
				// `${this.assetsPath}images/negx.jpeg`,
				// `${this.assetsPath}images/negy.jpeg`,
				// `${this.assetsPath}images/negz.jpeg`,
				// `${this.assetsPath}images/posx.jpeg`,
				// `${this.assetsPath}images/posy.jpeg`,
				// `${this.assetsPath}images/posz.jpeg`
			],
			oncomplete: function(){
				game.init();
			}
	...
	init() {
		...
		// ground
		var mesh = new THREE.Mesh( new THREE.PlaneBufferGeometry( 40000, 40000 ), new THREE.MeshPhongMaterial( { color: 0xffffff, depthWrite: false } ) );
	...
	playerControl(forward, turn){
		...
		if (forward>0.3){
			if (this.player.action!='walkingf' && this.player.action!='walkingf') this.player.action = 'walkingf';
		}else if (forward<-0.3){
	...
	onMouseDown( event ) {
		...
		if (intersects.length>0){
			...
			if (chat.style && (window.innerHeight - event.clientY)>40){
	...
	animate() {
		...
		if (this.player.action=='walkingf'){
			const elapsedTime = Date.now() - this.player.actionTime;
			if (elapsedTime>1000 && this.player.motion.forward>0){
				this.player.action = 'walkingf';
			}
		}
	...
	move(dt){
		...
		if (!blocked){
			if (this.motion.forward>0){
				const speed = (this.action=='walkingf') ? 500 : 150;
